...
public class DepositCalculator{
    ...
    double calculateComplexPercent(double a, double y, int d) {
        ...
        return valueRound(pay, 2);
    }

    double calculateSimplePercent(double doubleAmount, double double_year_rate, int deposit_period) {
        return valueRound(doubleAmount + doubleAmount * double_year_rate * deposit_period, 2);
    }

    double valueRound(double value, int places) {
    ...
    public static void main(String[] args){
        new DepositCalculator().depositCalculate();
    }
}
